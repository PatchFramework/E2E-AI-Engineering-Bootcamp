...
def search_filing_chunks_hybrid_impl(
    db: Session,
    query: str,
    company_id: int,
    ...
    """Internal implementation for search_filing_chunks_hybrid."""
    try:
        ...
        sql_base = """
            SELECT c.id, c.document_id, c.chunk_index, c.text_content, c.page_number, 
                   c.displayed_page_number, c.section_path, d.filename, d.fiscal_year
        ...
        if section_filter:
            sql_base += " AND c.section_path ILIKE :sec_filter"
        ...
        try:
            ...
            for idx, r in enumerate(rows):
                keyword_results.append({
                    ...
                    "section": r.section_path or "General",
                    "text": r.text_content,
                    "bounding_box": None,
        ...
        if not keyword_results:
            try:
                ...
                params_gen["fetch_limit"] = clamped_limit
                ...
                logger.warning(f"General search fallback encountered note: {e_gen}")
                db.rollback()
        ...
        logger.exception(f"Hybrid retrieval error for query '{query}': {e}")
        return {
            "status": "ERROR",
            ...
            "chunks": [],
            "citations": [],
            "suggestion": "Broaden the query or remove overly strict section filters."
        }

def count_concept_frequency_impl(
    db: Session,
    company_id: int,
    terms: List[str],
    document_id: Optional[int] = None
) -> Dict[str, Any]:
    """Internal implementation for count_concept_frequency."""
    try:
        sql = """
        ...
        logger.exception(f"Concept frequency error: {e}")
        return {
            "status": "ERROR",
            "error_type": type(e).__name__,
            "message": str(e),
...
def get_page_content_impl(db: Session, document_id: int, page_number: int) -> Dict[str, Any]:
    """Internal implementation for get_page_content."""
    try:
        ...
        logger.exception(f"Get page content error: {e}")
        return {
            "status": "ERROR",
            "error_type": type(e).__name__,
            "message": str(e),
...
def search_filing_chunks_hybrid(
    ...
    return search_filing_chunks_hybrid_impl(db, query, company_id, fiscal_year, section_filter, dense_weight, keyword_query, limit)
...
def count_concept_frequency(
    ...
    return count_concept_frequency_impl(db, company_id, terms, document_id)

def get_page_content(db: Session, document_id: int, page_number: int) -> Dict[str, Any]:
    return get_page_content_impl(db, document_id, page_number)

def create_retrieval_tools(db: Session) -> List[BaseTool]:
    """
    Creates LangChain @tool objects with `db` session bound via closure.
    """
    @tool
    @traceable(name="search_filing_chunks_hybrid", run_type="tool")
    def search_filing_chunks_hybrid_tool(
        query: str,
        company_id: int,
        fiscal_year: Optional[int] = None,
        section_filter: Optional[str] = None,
        ...
        """Executes Agentic Hybrid Retrieval combining semantic similarity and keyword matching across filing text."""
        return search_filing_chunks_hybrid_impl(db, query, company_id, fiscal_year, section_filter, dense_weight, keyword_query, limit)

    @tool
    @traceable(name="count_concept_frequency", run_type="tool")
    def count_concept_frequency_tool(
        company_id: int,
        terms: List[str],
        document_id: Optional[int] = None
    ) -> Dict[str, Any]:
        """Counts exact occurrence frequencies for financial keywords across filing chunks (for Word Cloud visualizations)."""
        return count_concept_frequency_impl(db, company_id, terms, document_id)

    @tool
    @traceable(name="get_page_content", run_type="tool")
    def get_page_content_tool(document_id: int, page_number: int) -> Dict[str, Any]:
        """Fetches raw text content and sections for an exact filing document page."""
        return get_page_content_impl(db, document_id, page_number)

    return [
        search_filing_chunks_hybrid_tool,
        count_concept_frequency_tool,
